...
#  "claude-sonnet-4-6"은 유효하지 않은 모델 ID라 404로 QnA·AI질의가 전부 실패했음
...
/* 상단 탭 그룹 구분: 분석(1~6) | 콘텐츠(QnA·성공사례, 7~8~). 7개 이상인 최상단 탭에만 적용
   (Streamlit 탭은 button이 아니라 [data-baseweb=tab] 요소 → 태그 지정 없이 nth-of-type) */
.stTabs [data-baseweb="tab-list"] [data-baseweb="tab"]:nth-of-type(7) {{
    margin-left:18px !important; padding-left:22px !important; border-left:1px solid {LINE} !important; }}
.stTabs [data-baseweb="tab-list"] [data-baseweb="tab"]:nth-of-type(7)[aria-selected="true"],
.stTabs [data-baseweb="tab-list"] [data-baseweb="tab"]:nth-of-type(8)[aria-selected="true"] {{
    color:{GOOD} !important; border-bottom-color:{GOOD} !important; }}
...
def render_landing_status():
    ...
    try:
        ...
        dot = (f'<span style="display:inline-block;width:8px;height:8px;border-radius:50%;'
               f'vertical-align:middle;margin-right:6px;background:{GOOD if fails == 0 else CORAL};"></span>')
        ...
        with st.expander(_label, expanded=False):
            ...
            if labels:
                ...
                for grp, g in sorted(cg.groupby("grp"), key=lambda kv: (bool(kv[1]["ok"].all()), kv[0])):
                    o = int(g["ok"].sum()); t = len(g)
                    gdot = (f'<span style="color:{GOOD}">●</span>' if o == t
                            else f'<span style="color:{CORAL}">●</span> {t-o}')
                    ...
                    for _, r in g.iterrows():
                        ...
                        emo = f'<span style="color:{GOOD if r["ok"] else CORAL}">●</span>'
...
def thin_xticks(fig, labels, target=10):
    """x축 라벨 정리.'YY.MM'(예:25.08)처럼 숫자로 보이는 라벨을 Plotly가 '수치축'으로
...
def roas_card(rev, ad, rev_p=None, ad_p=None, period="", show_profit=True, paid=None):
    ...
    #HTML은 반드시 '한 줄'로 — 줄바꿈·들여쓰기가 있으면 Streamlit 마크다운이 코드블록으로 오인해 태그가 글자로 노출됨
    html = (
...
def render_ai_chat():
    ...
    st.caption("AI 답변은 참고용입니다. 정확하지 않을 수 있으니 중요한 수치는 각 탭에서 확인하세요.")
...
def render_ga4():
    ...
    # ── 권한/데이터 가드 ──
    if info == "denied":
        st.warning("GA4 데이터셋(analytics_457680288) 읽기 권한이 없습니다.\n\n"
...
# ── 카테고리 성격 프로파일 (형사/민사/가사/행정/도산/복합) ──────────────────
#   프롬프트가 카테고리에 맞게 분기 → 비형사(민사·이혼·회생 등)에 '처벌' 프레임이 씌워지는 것 방지.
#   punish: "yes"=처벌 다룸 / "no"=형사개념 금지 / "cond"=실제 형사 요소일 때만.
QNA_PROFILES = {
...
# 모든 카테고리 공용 — 키워드가 가리키는 '사건 구도'를 모델이 임의로 바꾸지 못하게 하는 충실성 규칙.
#  (예: '상간남' 키워드인데 더 흔한 '상간녀' 상황으로 뒤집어 쓰는 오류 방지)
QNA_FIDELITY = (
...
def _qna_profile(cat):
    """카테고리 → 성격 프로파일. 미등록 카테고리는 안전한 중립 기본값."""
...
def _qna_forbid(prof):
    """punish 플래그 → 형사개념 금지/조건부 지시문."""
    p = prof.get("punish")
    if p == "no":
...
def _qna_bad_question(title, prof):
    ...
    # 왼쪽(키워드)이 '명백한 상황 서술문'일 때만 재생성(과도한 탈락 방지):
    #   물음표가 왼쪽 · 아주 김(45자↑) · 김(28자↑)+서술형 어미.
    kw_core = re.sub(r"\s*변호사$", "", kw).strip()
    if "?" in kw or len(kw_core) > 45 or (len(kw_core) > 28 and
            re.search(r"(했는데|하는데|였는데|당했|었어요|어요|아요|습니다|됐는데)", kw)):
        return True
...
def _qna_answer_prompt(title, keyword, cat, verified=None, ref=None):
    ...
    sysp = (f"너는 법무법인 KB의 변호사 원고를 쓰는 조수다. '{cat}' 분야 홈페이지 QnA 답변 '초안'을 만든다. "
            f"독자는 {prof['reader']}이며, 차분하고 정직한 톤으로 '{prof['stance']}' 관점에서 쓴다. "
            + _qna_forbid(prof) +
            ...
            "[수치 정확성] 법정형·형량·금액 등 구체 수치는 [검증된 법조문] 목록의 값에 있는 것만 그대로 인용하라. "
            "값이 없거나 '미기재'이면 구체적 숫자를 쓰지 말고 정성적으로만 서술하라(기억·추측으로 지어내지 마라). "
            "어느 경우든 단정하지 말고 '~할 수 있습니다'로 서술하라(개정 가능성 있음). "
            ...
            f"① 구체적으로: {prof['focus']}, 실제로 자주 벌어지는 상황과 흔한 실수 사례, "
            "단계마다 '무엇을 왜 하는지'를 담아라. '체계적으로 대응한다', '전문가의 도움이 필요하다' 같은 "
            "상투적·추상적 문장만 나열하지 마라. "
            ...
            "승소율·석방률 등 성과율 수치, '반드시·확실히 이긴다'식 보증, 판·검사 출신(前官) 영향력 암시, "
            "미검증 수상·순위 표현도 금지(변호사 광고규정 위반). "
...
# 지역판 대상 — 수도권(서울 자치구 + 인천 + 경기 시)으로 한정.
QNA_REGIONS = [
    # 서울 자치구
    "강남", "서초", "송파", "종로", "영등포", "마포", "강서", "노원", "관악", "성북", "동작", "광진", "은평",
    # 인천
    "인천", "부평",
    # 경기
    "수원", "성남", "용인", "부천", "안산", "안양", "화성", "평택", "의정부", "고양",
    "남양주", "시흥", "파주", "김포", "광명", "군포", "하남", "오산", "이천", "구리"]
...
def qna_localize_region(item, region, client=None):
    ...
    try:
        ...
        sysp = (f"너는 법무법인 KB의 지역 특화 QnA 편집자다. '{core}' 기본 원고를 "
                f"'{region}' 지역 독자용으로 자연스럽게 손본다. 규칙:\n"
                f"1) 사실·법조문·수치 내용은 절대 바꾸거나 새로 만들지 말 것.\n"
...
def qna_reco_keywords(cat, corpus, demand, n=10):
    ...
    sysp = (f"너는 법무법인 KB 홈페이지 QnA 게시판에 '다음에 쓸' 키워드를 추천한다. "
            ...
            "하나의 명확한 사건 주제만 담고, 서로 다른 개념을 억지로 붙이지 말 것(예: '상간남'과 '남편폭행'을 한 키워드로 합치지 말 것).\n"
...
def _qna_reset_item_flags():
   ...
   이걸 안 하면 인덱스가 겹치는 위젯(제목·문단·법조문 편집칸 `qna_e_*`, 상세선택 `qna_detail_sel`)에
...
def _qna_render_compare(res):
    ...
    st.caption("× = 검증 목록에 없는 조문(직접 확인 필요) · ✓ = 검증된 조문. "
               "Haiku가 미검증이 많거나 문단이 부실하면 품질 리스크가 큰 것.")
...
def qna_publish_daily(days=14):
   ...
   BigQuery는 한글 별칭(백틱 없이)을 거부 → SQL은 영문 별칭, 파이썬에서 한글로 rename."""
    try:
...
def qna_auto_pending():
    ...
    try:
        df = bq_fresh(f"""SELECT d.id, d.cat, d.title, d.payload, CAST(DATE(d.ts,'Asia/Seoul') AS STRING) dt
              ...
              ORDER BY d.ts DESC LIMIT 100""")   #한글 별칭은 BigQuery가 거부 → 영문 dt 사용
        if df is not None and not df.empty:
    ...
    if pend is None:
        ...
        for _, r in rows:
            ...
            with c1.expander(f"[{cat}]  {title}"):
                ...
                if st.button("이 원고 삭제(대기열에서 제거)", key=f"auto_skip_{did}"):
                    qna_skip_draft(did)
                    st.rerun()
...
def _qna_list_tab(corpus):
    ...
    sort = o2.selectbox("정렬", ["최신순", "오래된순", "분야별", "본문 짧은순(이상 찾기)", "중복 많은순"],
                        key="qna_list_sort")
    only_bad = o3.checkbox(f"이상·중복만 ({n_bad})", key="qna_list_bad")
...
def success_gen_case(cat, existing_titles=None, client=None):
    ...
    sysp = (
        ...
        "[GEO·정확성] 정확한 법령·조문명·수치·절차 단계를 근거로 담고, 핵심 근거 법령·판례 문구는 따옴표로 한 번 인용하라. "
        "[수치 원칙] 일반 법률정보 수치(법정형·공소시효·절차 기간·단계 수)는 구체적으로 쓰되, "
        "로펌의 '승소율·처리건수' 등 실적 통계는 절대 지어내지 마라(없는 실적 수치는 거짓광고). "
...
def success_gen_titles(cat, n=5, existing_titles=None, client=None):
    ...
    sysp = (
        ...
        f"[다양성 — 반드시] {n}개는 서로 **다른 죄명·다른 상황**이어야 하고, 결과도 목록에서 "
        "**최대한 다른 값으로 분산**하라(같은 결과·같은 전략에 몰지 마라). 특히 '합의'만 반복하지 말고 "
        "무혐의·불송치·기소유예·집행유예·감형·공탁·정상참작·구속기각 등 결과와 전략을 골고루 섞어라. "
        "과장·보증·승소율 표현 금지." + avoid +
        "\n[출력] JSON 배열만(설명 금지): "
...
def _qna_write_tab(corpus, cc, existing):
    ...
    man_txt = st.text_area("직접 쓸 키워드 (줄당 1개, 선택)",
    ...
    laws_txt = st.text_area("인용 법조문 (줄당 1개) — 편집 가능",
                            value="\n".join(ans.get("laws", [])),
            ...
            with st.spinner("법령 검색 중… (조문당 10초 내외)"):
                for l in need:
                    ...
                    if r.get("found") and qna_law_research_save(_u, cat, r):
                        ...
                        failed.append(l)
            if added:
                st.success(f"{added}건을 찾아노랑(검수 권장)으로 추가했습니다 — 검증목록·배지에 반영됩니다.")
            if failed:
    ...
    with st.expander(f"'{cat}'에서 인용 가능한 검증 법조문 {len(qna_laws_for(cat))}개"):
    ...
    if is_admin:
        ...
        with st.expander("기록 수정·삭제", expanded=False):
            if "id" not in df.columns:
                ...
                if bc[1].button("삭제", use_container_width=True, key=f"chg_del_{cat}", disabled=not del_ok):
...
def main():
    ...
    if user == "admin":
        lo = st.columns([2.6, 1.5, 1.3, 1.3, 1.3])   # 세 버튼 동일 폭(1.3)
        _ucol, _xcol, _rcol, _gcol = lo[1], lo[2], lo[3], lo[4]
    else:
        lo = st.columns([3.6, 1.5, 1.3, 1.3])         # 두 버튼 동일 폭
    ...
    if is_admin:
        with top[len(top_labels) - 1]:            # 신규KW(맨 뒤, 관리자 전용)
...
try:
    ...
    st.error("앱 실행 중 오류가 발생했습니다. 아래 빨간 상자를 통째로 캡처해서 보내주세요.")

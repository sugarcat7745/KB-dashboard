...
GEO_RULES = ("\n[GEO — 생성형 AI가 인용하기 좋은 글 (2025 실증연구: 통계·출처·직접인용이 인용률 30~40%↑)]\n"
             "① 직답형: 결론부터, 각 문단이 그 자체로 완결된 답이 되게.\n"
             "② 구체 수치(최대 인용요인): 모호한 표현 대신 정확한 숫자·기간·비율·범위·단계 수(예: '5년 이하 징역', "
             "'공소시효 7년', '3단계'). 단, 로펌 승소율·처리건수 등 실적 통계는 지어내지 마라(거짓광고).\n"
             "③ 출처 인용: 정확한 법령명·조문번호와 근거를 명시.\n"
             "④ 직접 인용: 핵심 근거 법령·판례 문구를 따옴표로 한 번 인용.\n"
             "⑤ 정확한 용어·개체명, 홍보성 미사여구 줄이기. 검증 안 된 수치는 지어내지 않는다.")

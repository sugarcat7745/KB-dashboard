...
3. `plan.json` 을 읽고, **직접 원고를 생성**한다. 반드시 지킬 것:
   ...
   - **자기완결 캡슐**(rules.capsule): 각 섹션 첫 문단은 그 문단만 읽어도 완결되는 40~70자 직답으로
     시작하고, 사실·수치 문장엔 근거 조문을 문장 안에 붙인다(AI는 문단째 인용한다).
   - **정의형 문장**(rules.definition): 각 글에 핵심 용어 1문장 정의를 최소 1개 포함.
   - **유사중복 금지**(rules.no_duplicate): avoid_titles 와 키워드·주제가 겹치는 사실상 같은 글 금지.
